...
def sigma_crit(d_a_lens, d_a_source, d_a_lens_source) :
    ...
    return constants.c.to('Mpc/s') * constants.c.to('Mpc/s') / \
        (4. * np.pi *  constants.G.to('Mpc3 / (Msun  s2)') * \
         d_a_lens * beta_function(d_a_source, d_a_lens_source) )

def beta_function(d_a_source, d_a_lens_source) :
    """
...
def beta_discrete_mean(d_a_lens, d_a_source, d_a_lens_source) :
    """
    ...
    if np.iterable(d_a_source) and all(isinstance(d_a_s, float) for d_a_s in d_a_source) :
        return np.mean(beta_function(d_a_source, d_a_lens_source))

    elif isinstance(d_a_source, float) :
        return beta_function(d_a_source, d_a_lens_source)
...
def beta_square_discrete_mean(d_a_lens, d_a_source, d_a_lens_source) :
    """
    ...
    if np.iterable(d_a_source) and all(isinstance(d_a_z, float) for d_a_z in d_a_source) :
        return np.mean(beta_function(d_a_source, d_a_lens_source)**2.)

    elif isinstance(d_a_source, float) :
        return beta_function(d_a_source, d_a_lens_source)**2.
